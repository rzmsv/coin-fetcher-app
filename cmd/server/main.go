...
import (
	...
	"github.com/username/coin-fetcher-app/internal/adapters/repository"
...
func main() {
	...
	repository := repository.NewRepository(appConfigs, db)
	priceFetcher := external.NewCoinGeckoFetcher()
	priceService := application.NewPriceService(repository, priceFetcher)
